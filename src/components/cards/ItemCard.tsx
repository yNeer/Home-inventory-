...
export const ItemCard: React.FC<ItemCardProps> = ({ item, onDelete }) => {
  ...
  return (
    <div className="relative overflow-hidden bg-white/80 backdrop-blur-xl rounded-[32px] p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/40 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 transform hover:-translate-y-1">
      {/* Glow effect */}
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="flex items-center gap-4">
          <div className={`flex items-center justify-center w-[52px] h-[52px] rounded-[20px] shadow-sm backdrop-blur-md ${isMedicine ? 'bg-gradient-to-br from-rose-100 to-rose-50 text-rose-500 border border-rose-200/50' : 'bg-gradient-to-br from-indigo-100 to-indigo-50 text-indigo-500 border border-indigo-200/50'}`}>
            {isMedicine ? <FaPills size={24} /> : <FaBox size={24} />}
          </div>
          <div>
            <h3 className="font-[700] text-[19px] text-slate-800 capitalize leading-tight mb-1 font-sans tracking-tight">{item.name}</h3>
            <div className="flex flex-wrap items-center gap-2">
              <span className={`text-[10px] uppercase font-bold tracking-widest px-2.5 py-1 rounded-full bg-slate-100 text-slate-500`}>{item.type}</span>
              {item.price && <span className="text-[11px] font-bold text-slate-600 px-2 py-0.5 rounded-full border border-slate-200">₹{item.price}</span>}
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-3 bg-slate-50/50 rounded-[24px] p-3.5 border border-slate-100/50 relative z-10">
        <div className="flex-1">
          <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
             <FaRegClock size={10} /> Mfg
          </span>
          <span className="font-semibold text-slate-700 text-[13px]">{item.mfgDate ? format(new Date(item.mfgDate), 'MMM dd, yyyy') : 'Unknown'}</span>
        </div>
        ...
        <div className="flex-1 pl-2">
          <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
             <FaRegClock size={10} /> Exp
          </span>
          <span className={`font-bold text-[13px] ${status.textCol}`}>
